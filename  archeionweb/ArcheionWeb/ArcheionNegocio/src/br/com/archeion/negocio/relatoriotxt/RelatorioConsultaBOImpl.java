...
public class RelatorioConsultaBOImpl implements RelatorioConsultaBO {
	...
	public List<EtiquetaPastaBean> geraEtiquetaPastaPorCaixa(String ids) {
		...
		return realizaConsulta(sql.toString());
	}
	...
	public List<EtiquetaPastaBean> geraEtiquetaPastaPorCaixeta(String ids) {
		...
		sql.append(" where p.id_pasta in ( ");
		sql.append(ids);
		sql.append(" ) order by  p.nm_caixeta ");

		return realizaConsulta(sql.toString());
	}

	private List<EtiquetaPastaBean> realizaConsulta(String sql) {
		Connection conn = relatorioDAO.getConnection();
		...
		try {
			...
			while (rs.next()) {
				...
				etiqueta.setPrevisaoRecolhimento((rs.getDate(5) != null) ? df
						.format(rs.getDate(5)) : "");
				etiqueta.setPrevisaoExpurgo((rs.getDate(6) != null) ? df
						.format(rs.getDate(6)) : "Permanente");
				etiqueta.setCaixaOuCaixeta((rs.getString(9) != null) ? ("Caixa"
						+ rs.getString(9) + rs.getString(8))
						: ("Caixeta: " + rs.getString(7)));
